...
var allowCrossDomain = function (req, res, next) {
  ...
  res.header(
    ...
    "Content-Type, Content-Length, Authorization, X-Powered-By, Accept,X-Requested-With"
  );
...
app.get("/", function (req, res, next) {
  ...
  let url = `https://dcm.qq.com/assets/query/${productId}${
    contractId ? `/${contractId}` : ""
  }?subMerchantId=0&sck=${sck}&r=${new Date().getTime()}`;
  ...
  superagent
    ...
    .end(function (err, sres) {
      ...
      let {
        contractProductInfos,
        balanceList,
        contractAcctInfos,
        cdkeyContractAcctInfos,
        rebateGoodsAcctInfos,
        rebateGoodsBalanceList,
        rebateMoneyWaters,
      } = resData.info;
      let rule = [],
        rebateGoodsRule = [];
      ...
      contractProductInfos.map((item) => {
        // 组合需要匹配的数据
        let FJifenProductList =
          item.contractProduct.FJifenOfferID +
          "," +
          item.contractProduct.FJifenProductID;
        // 除的价格
      ...
      rule.map((item) => {
        item.map((innerItem, index) => {
          balanceList.map((banlitem) => {
            // 计算返回结果普通数据
            if (
              innerItem.FJifenProductList === banlitem.FJifenProductList &&
              index === banlitem.FGradient
            ) {
            ...
            // 计算返回结果CDK数据
            if (
              innerItem.FJifenProductList === banlitem.FJifenProductListCDK &&
              index === banlitem.FGradientCDK
            ) {
              ...
              // 计算剩余的
              innerItem.calcBanlaceCDK = accDiv(
                banlitem.balance,
                innerItem.price
              );
            }
      ...
      rule.map((item) => {
        ...
        item.map((innerItem) => {
          ...
          innerItem.calcBanlaceCDK &&
            (calcAddBanlaceCDK += innerItem.calcBanlaceCDK);
        });
        nowAllArr.push({
          calcAddAllIn,
          calcAddBanlace,
      ...
      rule.map((item) => {
        item.map((nowItem) => {
          rebateGoodsBalanceArr.map((innerItem) => {
            // 计算返回结果
            if (
              innerItem.FJifenProductListRebase === nowItem.FJifenProductList
            ) {
              // 计算所有的
              innerItem.FRebatePrice = nowItem.FRebatePrice;
              // 计算所有的
              innerItem.calcAllIn = accDiv(
                innerItem.all_in,
                nowItem.FRebatePrice
              );
              // 计算所有的
              innerItem.FName = `返货（${nowItem.FName}）`;
              // 计算剩余的
              innerItem.calcBanlace = accDiv(
                innerItem.balance,
                nowItem.FRebatePrice
              );
            }
...
app.get("/getList", function (req, res, next) {
  const { wholesale_sid, sck, businessGroupId, id } = req.query;
  const obj = {
    outputType: "json",
    type: "money",
    contractId: "",
  ...
  let url = `https://dcm.qq.com/rebate/list/${id}`;
  superagent
    .post(url)
    .send({
      ...obj,
